...
def get_model():
    model = convnext.convnext_small(weights=convnext.ConvNeXt_Small_Weights.IMAGENET1K_V1)
...
def train(model, train_loader, val_loader, n_epochs=1, fname="models/resnet50.pth"):
    ...
    scheduler = MultiStepLR(optimizer, [15, 20, 30, 40])
...
if __name__ == "__main__":
    ...
    model = get_model()
    train(model, train_loader, val_loader, n_epochs=50, fname="models/convnext-small-last.pth")
    predict(model, dfpath="data/sample_submission.csv", imgroot="data/test/",
            outpath="data/convnext-small-last.csv")
