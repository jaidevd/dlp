...
def shuffle_split(train_ds):
    _, labels = zip(*train_ds)
    trix, valix = train_test_split(range(len(train_ds)), test_size=0.25, stratify=labels)
    train_ds, val_ds = Subset(train_ds, trix), Subset(train_ds, valix)
...
def get_loaders(root):
    ...
    trix, tsix = train_test_split(range(len(ds)), test_size=0.2, stratify=ds.datasets[1])
    train_ds, test_ds = Subset(ds, trix), Subset(ds, tsix)
    train_loader, val_loader = shuffle_split(train_ds)
...
if __name__ == "__main__":
    train_loader, val_loader, test_loader, train_ds = get_loaders("data/train/")
    model = get_model()
    for i in range(5):
        train(model, train_loader, val_loader, n_epochs=5, fname="models/resnet50-last3-best.pth")
        train_loader, val_loader = shuffle_split(train_ds)

    # Test the model, unseen data so far.
    model.eval()
    losser = nn.CrossEntropyLoss()
    y_test_pred = []
    y_test_true = []
    ...
    for images, labels in test_loader:
        with torch.no_grad():
            out = model(images)
        y_test_pred.extend(out.argmax(dim=1).tolist())
        y_test_true.extend(labels.tolist())
        loss = losser(out, labels)
        test_loss += loss.item()
    test_loss = round(test_loss / len(test_loader), 3)
    val_f1 = round(f1_score(y_test_true, y_test_pred, average="macro"), 2)
    print(f'Test loss: {test_loss}; Test F1: {val_f1}')  # NOQA: T201
