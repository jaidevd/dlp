...
def collate(batch, device, resize=True):
    ...
    if resize:
        images, bboxes = zip(
            *[resize_with_ar(im, t["boxes"][0]) for im, t in zip(images, targets)]
        )
        new_targets = []
        ...
        for epoch in range(n_epochs):
            ...
            with tqdm(
                ...
                for images, targets in train_loader:
                    ...
                    opt.step()
                    bbar.update(1)
                    bbar.set_postfix_str(f"Train(loss={btl:.3f})", refresh=True)
...
def make_submission(
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        df.loc[
            idx, ["LabelName", "Conf", "xcenter", "ycenter", "bbx_width", "bbx_height"]
        ] = [label, conf, xcenter, ycenter, box_width, box_height]
    if outpath is None:
    ...
    if show:
        ...
        for (idx, row), ax in zip(xdf.iterrows(), ax.ravel()):
            ...
            xcenter, ycenter, box_width, box_height = row[
            ...
            ax.set_axis_off()
            ax.set_title(
                f"{row['LabelName']} ({round(row['Conf'], 2)})", fontsize="small"
            )
        plt.tight_layout()
